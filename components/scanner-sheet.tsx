 ...
 * above the tab bar — the operating session stays live in both states,
 * and `layoutId` morphs one into the other.
 */
...
export function ScannerSheet({ showBar }: { showBar: boolean }) {
  ...
  // Keyed to its event so a count never carries over to the next one.
...
function OperatingBar({
  ...
  return (
    <motion.div
      ...
      className="fixed inset-x-4 bottom-[calc(max(env(safe-area-inset-bottom),1rem)+4.6rem)] z-40 mx-auto flex max-w-md items-center gap-2 bg-stone-900 pl-4 pr-2 py-1.5 text-white shadow-float"
    >
      <button
        onClick={onExpand}
...
function ExpandedScanner({
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-50 flex flex-col overflow-hidden bg-stone-950 text-white"
    >
      ...
      {/* Top bar */}
      <div className="z-10 flex items-center gap-2 px-3 pb-2 pt-2">
        <button
          onClick={onCollapse}
      ...
      {/* Bottom bar */}
      <div className="z-10 shrink-0 space-y-2 px-3 pb-[max(env(safe-area-inset-bottom),0.75rem)] pt-2">
